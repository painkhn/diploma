    ...
    <ul class="space-y-4">
        <li v-for="(item, index) in props.projectUsers" :key="index">
            <div class="grid grid-cols-2 items-center px-5">
                ...
                <div
                    class="w-10 h-10 flex items-center justify-center rounded-full border-2 overflow-hidden dark:border-white/10">
                ...
                <div class="flex flex-col text-sm">
                    <span class="font-semibold text-base">{{ item?.user.name }}</span>
                    <span v-if="item?.role === 'admin'">
                        Администратор
                    </span>
                    <span v-if="item?.role === 'moderator'">
                        Модератор
                    </span>
                    <span v-if="item?.role === 'inspector'">
                        Проверяющий
                    </span>
                ...
                <div class="justify-self-end">
                    ...
                    <ActionsDropdown v-if="currentProjectUser.role === 'admin' && item.role !== 'admin'" :project="props.project"
